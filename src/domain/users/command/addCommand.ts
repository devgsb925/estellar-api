...
import { IAuthenticationResponse } from '../queries/dto/i-authentication-response'
...
const addCommand = async (model: IUser) => {
    ...
    var uidExists = await (await UserService.GetUserByIdService(uid)).rowCount;
    if (uidExists > 0) {
        uid = GenerateUUID();
    }
    ...
    // add user role
    var userRoleResponse = await UserService.AddUserRoleService(uid, process.env.ADMINISTRATION_UUID, true);
    if (!userRoleResponse) {
    ...
    const responseData: IAuthenticationResponse = {
