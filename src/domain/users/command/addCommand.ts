...
import { GenerateSalt, RandomStringGenerator, PasswordHasher, GenerateUUID } from '../../../utility/cryptography';

const addCommand = async (model: IUser) => {
  const userNameExists = (await UserService.GetUserByUsername(model.username)).rowCount;
  if (userNameExists > 0) {
  ...
  const uidExists = await (await UserService.GetUserById(uid)).rowCount;
  if (uidExists > 0) {
    uid = GenerateUUID();
  }
  ...
  const response = await UserService.AddUser(model);
  if (!response) {
    throw new AppError(
  ...
  // add user role
  const userRoleResponse = await UserService.AddUserRole(uid, process.env.ADMINISTRATION_UUID, true);
  if (!userRoleResponse) {
    throw new AppError(
      ErrorCodes.FAILED_POST_DATA_ERROR.errorCode,
      ErrorCodes.FAILED_POST_DATA_ERROR.description,
      ErrorCodes.FAILED_POST_DATA_ERROR.statusCode,
    );
  }
